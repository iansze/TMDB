...
const AuthForm = ({ fields, onSubmit, buttonText, title }: AuthFormProps) => {
  ...
  return (
    <div className="relative h-screen bg-netflix-background bg-cover bg-center object-contain ">
      ...
      <div className="absolute inset-0  top-[30%] mx-auto max-w-screen-sm ">
        <form
          onSubmit={handleSubmit(onSubmit)}
          className=" flex flex-col rounded bg-zinc-950 p-8  "
        >
          <h1 className="mx-auto mb-7 text-3xl font-bold text-white ">
            {title}
          </h1>
          {fields.map((field) => (
            <input
              ...
              className="mx-auto mb-4 w-3/4 rounded-sm p-2"
          ...
          <button
            type="submit"
            className="mx-auto mb-4 mt-4 w-3/4 rounded-sm bg-red-600  p-2 text-white"
          >
            {buttonText}
          </button>
          {title === "Sign In" && (
            <div className="text-center text-xs text-white sm:text-base">
              New to Netflix?
